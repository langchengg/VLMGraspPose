...
| VLM target mode | PARTIAL | Optional interface implemented. Florence-2 backend wrapper exists, but weights are not bundled or verified here. |
...
| Proxy evaluation | COMPLETE | Top-K proxy validity and grouped metrics. |
...
Overall status: COMPLETE for oracle/rule-based/optional-MLP CPU experiments; PARTIAL for real VLM grounding until Florence-2/SAM dependencies and weights are installed locally.
