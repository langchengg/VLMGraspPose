"""Freeze the audited primary-plan datasets.py compatibility sidecar."""
...
from d1_reranking.primary_source_adapter import (  # noqa: E402
    ADAPTER_RELATIVE,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--refresh",
        action="store_true",
        help="Archive a prior prelock adapter and publish the current audited version.",
    )
...
def main() -> int:
    ...
    with ledger_stage(
        ...
        stage="P13",
        substage="d1_primary_source_adapter_v1",
        route="D1",
        pool="top5",
        evidence_track="T2_matched_common",
        method="primary_360_job_exact_rebuild",
        command=" ".join(map(str, sys.argv)),
    ) as state:
        value = prepare_primary_source_adapter(
            root, resume=args.resume, refresh=args.refresh
        )
