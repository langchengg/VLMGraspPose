    ...
    for kdx in range(masks.shape[0]):
        ...
        for y in range(arr_h):
            for x in range(arr_w):
                ...
                arr_run[y * p_h:(y + 1) * p_h, x * p_w:(x + 1) * p_w] = (running_total_nonlin > 0.5).astype(float)
...
def _candidate_checksum(candidate):
    """Hash only immutable grasp geometry, not scores or ranks."""
    payload = {
        "row": candidate["row"],
        "col": candidate["col"],
        "cx": candidate["cx"],
...
def detect_grasp_candidates(
    ...
    """Return CROG's unchanged local-peak candidates with auditable metadata.

    Coordinates returned by ``peak_local_max`` are ``(row, column)``.  CROG's
    public grasp representation remains ``(x=column, y=row, width, height,
    angle_degrees)``.  Candidate generation parameters deliberately match the
    legacy implementation exactly.
    """
    quality = np.asarray(grasp_quality_mask)
    ...
    candidates = []
    for legacy_rank, p_array in enumerate(local_max):
        row, col = (int(p_array[0]), int(p_array[1]))
        angle_rad = float(grasp_angle_mask[row, col])
        angle_deg = angle_rad / np.pi * 180.0
        width_px = float(grasp_wid_mask[row, col] * 100)
        height_px = 20
        legacy_grasp = [float(col), float(row), width_px, height_px, angle_deg]
        polygon_points = cv2.boxPoints(
            ((float(col), float(row)), (width_px, float(height_px)), -angle_deg)
        )
        candidate = {
            "candidate_id": f"candidate_{legacy_rank}",
            ...
            "width_px": width_px,
            "height_px": height_px,
            "polygon": polygon_points.astype(float).tolist(),
            "q_raw": float(quality[row, col]),
            "legacy_grasp": legacy_grasp,
        }
    ...
    q_order = sorted(
        ...
        key=lambda index: (-candidates[index]["q_raw"], candidates[index]["legacy_rank"]),
    )
    for q_rank, candidate_index in enumerate(q_order):
        candidates[candidate_index]["q_rank"] = q_rank
    return candidates, grasp_angle_mask


def detect_grasps(grasp_quality_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, num_grasps=5):
    """Backward-compatible CROG grasp detector."""
    candidates, grasp_angle_mask = detect_grasp_candidates(
        grasp_quality_mask,
        grasp_sin_mask,
        grasp_cos_mask,
        grasp_wid_mask,
    ...
    return [candidate["legacy_grasp"] for candidate in candidates], grasp_angle_mask
...
def calculate_iou(
    ...
    *,
    ...
    """Return pair IoU only when that same pair passes the angle threshold.

    This compatibility wrapper is intentionally backed by the canonical,
    versioned evaluator.  New code that needs diagnostics should call
    :func:`utils.grasp_metrics.evaluate_candidate` directly.
    """
    ...
    if not result["candidate_success"]:
        return 0.0
    return float(result["best_gt"]["rectangle_iou"])
...
def calculate_jacquard_index(
    ...
    *,
    ...
    """Return J@N without mutating the supplied ground-truth array."""
    return int(
        grasp_set_success(
            list(grasp_preds),
            grasp_targets,
