...
from gtmask_counterfactual.mapping import build_prelock_registry
    ...
    for route in ("g1", "c1", "d1"):
        route_sources[route] = [
            ...
            *(
                []
                if route == "c1"
                else [{**rows[1], "source_identity": f"{route}:s2"}]
            ),
        ]
...
def test_bulk_pixel_qa_remains_pending_without_manual_csv(tmp_path: Path) -> None:
    ...
    manifest = {
        **_manifest_row(sample_id),
        ...
        "source_instance_mask_sha256": sha256_file(paths["instance"]),
