...
from .reporting import REPORT_NAMES, TABLE_CONTRACTS
...
def _check_figures(root: Path, *, partial: bool) -> dict[str, Any]:
    ...
    expected_names = 11 if partial else 12
    expected_prefixes = REQUIRED_FIGURE_STEMS[:expected_names]
    if (
        manifest.get("status") != ("PARTIAL" if partial else "COMPLETE")
        ...
        or (partial and any(str(name).startswith("12_") for name in figures))
    ):
...
def _check_protocol_execution(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
...
def _check_gallery(root: Path) -> dict[str, Any]:
    ...
    for record_name in ("eligible", "selected"):
...
def _check_routes(
    ...
    if not isinstance(routes, Mapping) or set(routes) != {"G1", "C1", "D1"}:
        raise ValueError("route status must cover exactly G1/C1/D1")
    ...
    d1_status = routes.get("D1")
    if d1_status == "COMPLETE":
        if d1_blocker is not None:
            raise ValueError("D1 cannot be both COMPLETE and blocked")
        return "COMPLETE", route_status
    if d1_status != "UNRECOVERABLE_BLOCKER" or d1_blocker is None:
        raise ValueError("D1 must be COMPLETE or carry a legal unrecoverable blocker")
    if (
        d1_blocker.get("status") != "UNRECOVERABLE_BLOCKER"
        or d1_blocker.get("blocker_class")
        != "IRRECOVERABLE_FROZEN_SOURCE_EVIDENCE"
        or d1_blocker.get("raw_candidate_regeneration_required") is not True
    ):
    ...
    if missing or any(
        not value
        for value in (
            d1_blocker.get("missing_evidence"),
            d1_blocker.get("search_paths"),
            d1_blocker.get("stack_trace"),
            d1_blocker.get("resume_command"),
        )
    ):
        raise ValueError(f"D1 blocker evidence is incomplete: {missing}")
    if d1_blocker.get("filter_only_sensitivity_used_as_primary") is not False:
        raise ValueError("filter-only sensitivity cannot substitute for D1 primary")
    return "PARTIAL", route_status
...
def finalize_run(
    ...
    """Write COMPLETE only for a fully complete three-route scientific run.

    A legal D1 raw-generation replay blocker produces a terminal PARTIAL lock
    and PARTIAL marker.  It can never create a COMPLETE marker.
    """
    ...
    _protocol, execution = _check_protocol_execution(root)
    ...
    transition_pipeline_status(
        root,
        RunState.PARTIAL if partial else RunState.COMPLETE,
    ...
    payload: dict[str, Any] = {
        ...
        "counterfactual_execution": artifact_record(
...
def verify_final_lock(run_dir: str | Path) -> dict[str, Any]:
    ...
    if status == "COMPLETE" and lock.get("d1_unrecoverable_blocker") is not None:
        raise ValueError("COMPLETE lock carries a D1 blocker")
    excluded = [
        lock_path,
        digest_path,
        root / "COMPLETE",
