"""Official pinned GG-CNN2 depth-only inference adapter."""
...
from ..common import GraspPrediction, decode_quality_maps, non_maximum_suppression
...
class GGCNN2Config(NetworkBackendConfig):
    checkpoint_path: Path = GGCNN2_CHECKPOINT
    checkpoint_sha256: str = GGCNN2_CHECKPOINT_SHA256


class GGCNN2Backend:
    """Frozen one-channel official model behind the uniform predict contract."""

    backend_name = "ggcnn2"

    def __init__(
        self,
        config: GGCNN2Config = GGCNN2Config(),
        *,
        ...
        if model is None:
            self.model, self.checkpoint_sha256 = load_ggcnn2_state_dict(
    ...
    def predict(self, sample: BackendSample) -> GraspPrediction:
        ...
        if model_input.shape != (1, self.config.input_size, self.config.input_size):
            return self._empty(
                sample,
