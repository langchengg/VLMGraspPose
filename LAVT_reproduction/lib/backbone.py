...
import numpy as np
from .compat import DropPath, get_root_logger, load_checkpoint, to_2tuple, trunc_normal_
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint and torch.is_grad_enabled():
                # Explicitly preserve the re-entrant checkpoint behavior used
                # by the original PyTorch version and avoid version-dependent
                # defaults in current PyTorch releases.
                x = checkpoint.checkpoint(
                    blk, x, attn_mask, use_reentrant=True
                )
            else:
