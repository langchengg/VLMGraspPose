...
def criterion(input, target):
    weight = torch.tensor([0.9, 1.1], dtype=input.dtype, device=input.device)
...
def evaluate(model, data_loader, bert_model, device):
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            image, target, sentences, attentions = image.to(device, non_blocking=non_blocking),\
                                                   target.to(device, non_blocking=non_blocking),\
                                                   sentences.to(device, non_blocking=non_blocking),\
                                                   attentions.to(device, non_blocking=non_blocking)
...
def main(args):
    if args.device == "auto":
        if torch.cuda.is_available():
            device = torch.device("cuda", args.local_rank or 0)
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=(args.pin_mem and device.type == "cuda"), drop_last=True)
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, epoch, args.print_freq,
                        iterations, bert_model, device)
        iou, overallIoU = evaluate(model, data_loader_test, bert_model, device)
